...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let fieldPublicKey = unitpayForm.querySelector('[data-unitpay="publicKey"]');
    ...
    if (!!fieldPublicKey) {
        ...
        fieldPublicKey.addEventListener('change', function (event) {
            let index = this.selectedIndex;
            let options = this.querySelectorAll('option');
            let selectedOption = options[index];
            let price = selectedOption.getAttribute('data-unitpay-price');
            fieldPrice.value = price;
            fieldCount.dispatchEvent(new Event("input"));
        });
        fieldPublicKey.dispatchEvent(new Event("change"));
